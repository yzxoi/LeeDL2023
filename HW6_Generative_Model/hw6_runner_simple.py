	...
	def __init__(
		...
		*,
		...
		if self.accelerator.is_main_process:
			# initialize wandb
			os.environ['HTTP_PROXY']  = 'http://192.168.31.69:7890'
			os.environ['HTTPS_PROXY'] = 'http://192.168.31.69:7890'
			wandb.init(
			...
			while self.step < self.train_num_steps:
				...
				if accelerator.is_main_process:
					...
					if self.step != 0 and self.step % self.save_and_sample_every == 0:
						...
						img_path = str(self.results_folder / f'sample-{milestone}.png')
						wandb.log({
							"samples": [wandb.Image(img_path, caption=f"Sample {milestone}")],
						}, step=self.step)
...
trainer = Trainer(
	...
	path,
	wandb_project= "hw6_diffusion",
	wandb_run_name = "diffusion_run_simple",
...
# %%
ckpt = './hw6/simple_baseline/model.pt'
trainer.load(ckpt)
trainer.inference()
